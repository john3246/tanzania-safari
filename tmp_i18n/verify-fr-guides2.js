...
const authorMarker = ['$', '{', 'AUTHOR.whatsapp', '}'].join('');
const enHints = [
  'How much does',
  'Best Time to Visit',
  'In This Guide',
...
for (const f of needed) {
  ...
  if (!j.html || j.html.length < 500) issues.push('html short');
  if ((j.faqs || []).length !== (en.faqs || []).length) {
    issues.push('faqs ' + (j.faqs || []).length + '!=' + (en.faqs || []).length);
  }
  if ((j.html || '').includes(authorMarker)) issues.push('AUTHOR leftover');
  const found = enHints.filter((h) => (j.html || '').includes(h));
  if (found.length) issues.push('EN: ' + found.join('|'));
  if (j.title === en.title) issues.push('title EN');
  console.log(
    f,
    'html=' + (j.html || '').length,
    'faqs=' + (j.faqs || []).length,
